...
WP_VOLUME_ACCESSMODE="ReadWriteMany"
...
install.sh --namespace=project-namespace --host=mysite.com [--wp-image=$WP_IMAGE] [--db-image=mariadb:10.3]
    ...
    --wp-volume-accessMode   acess mode do volume do wordpress (padrão: $WP_VOLUME_ACCESSMODE)
...
case $i in
    ...
    --wp-volume-sc=*)
	    ...
	    shift
    ;;
    --wp-volume-accessmode=*)
            WP_VOLUME_ACCESSMODE="${i#*=}"
	    shift
    ;;
    --db-volume-sc=*)
...
- WP Volume Access Mode        $WP_VOLUME_ACCESSMODE
...
helm install --namespace=$NAMESPACE --set "image=$WP_IMAGE,db.image=$DB_IMAGE,ingress.hosts[0].host=$HOST,ingress.hosts[0].paths[0]='/',ingress.secretName=$NAMESPACE-crt,persistence.size=$WP_VOLUME_SIZE,persistence.storageClass=$WP_VOLUME_STORAGE_CLASS,persistence.accessMode=$WP_VOLUME_ACCESSMODE,db.persistence.size=$DB_VOLUME_SIZE,db.persistence.storageClass=$DB_VOLUME_STORAGE_CLASS" $NAMESPACE ./